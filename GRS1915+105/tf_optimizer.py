    ...
    def __init__(
        ...
        if type(norm) == str:
            if norm not in _NORMS:
                raise ValueError('Provided norm `{}` must be one of `{}`.'.format(norm, _NORMS.keys()))
...
class AdaptiveNormalizedSGD(tf.train.Optimizer):
    ...
    def __init__(
        ...
        if norm_type not in ['max', 'l2', 'std']:
            raise ValueError('Unexpected norm type `{}`.'.format(norm_type))
...
class BarzilaiBorweinNormalizedSGD(tf.train.Optimizer):
    ...
    def __init__(
        ...
        if norm not in ['max', 'l2']:
            raise ValueError('Unexpected norm type `{norm}`.'.format(norm))
